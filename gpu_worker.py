...
class GpuWorkerThreads(QtCore.QObject):
    ...
    fan_curce_thread = QtCore.pyqtSignal(int, str, str, int)


class GpuStats(QtCore.QRunnable):
    def __init__(self):
...
class GpuFanCurve(QtCore.QRunnable):
    def __init__(self):
    ...
    def run(self):
        ...
        while True:
            if show_gpu_vendor == repr('GeForce'):
                ...
                self.fcurve_signal.fan_curce_thread.emit(gpu_temp, speed, show_gpu_vendor, max_fan)
                QtCore.QCoreApplication.processEvents()
                time.sleep(3)
            elif show_gpu_vendor == repr('AMD'):
                import amdgpu
